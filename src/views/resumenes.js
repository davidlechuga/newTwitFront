...
function Resumenes() {
	...
	useEffect(() => {
		fetch('https://servetwit.mybluemix.net/resumenes').then((resp) => resp.json()).then((resp) => {
			setState({
				twits: [`${JSON.stringify(resp.result.keywords)}`],
				hasError: false,
				errorMessage: ''
			});
		});
	...
	console.log(`HOLA SOY YOO !!! :D  ${JSON.stringify(state.twits)}`);
		...
		<main>
			{!state.hasError ? (
				<React.Fragment>
					...
					{state.twits.map((t, index) => (
						<li key={index}>
							<h1> {t} </h1>
							{/* <ResumenCard
								text={t.result.keywords.text}
								relevance={t.result.keywords.relevance}
								emotion={JSON.stringify(t.result.keywords.emotion)}
							/> */}
						</li>
					))}

					{/* <StatCard emotion={state.twits} /> */}

					<Footer />
				</React.Fragment>
			) : (
					<div>
						<h1 className="title is-error">Ups parece que algo salió mal al consultar al back :(</h1>
