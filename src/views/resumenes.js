...
function Resumenes() {
	...
	useEffect(() => {
		fetch('https://serveanalystwit.mybluemix.net/resumenes').then((resp) => resp.json()).then((resp) => {
			setState({
				twits: resp,
				hasError: false,
				errorMessage: ''
	...
	console.log(`HOLA SOY YOO !!! :D ${setState.twits}`);
		...
		<main>
			{!state.hasError ? (
				<React.Fragment>
					...
					{state.twits.map((t) => (
						<li key={t}>
							<ResumenCard text={t.text} relevance={t.relevance} emotion={JSON.stringify(t.emotion)} />
						</li>
					))}
